...
namespace nht.barcodes._1D
{
	public class EAN13
	{
		...
		public BarcodeResult Encode(string data)
		{
			...
			{
				Image = BarcodeDraw(_sb.ToString()),
				Barcode = $"{data}{cs}"
			};
		}

		private Image BarcodeDraw(string barcode)
		{
			Image image = new Bitmap(226, 100);
			...
			var ddd = (double)226 / barcode.Length;

			var pointer = 0;

			for (var position = 0.0; position < 226; position += ddd)
			{
				if (pointer < barcode.Length && barcode[pointer] == '1')
				{
					graph.FillRectangle(Brushes.Black, (float)position, 0, (float)ddd, 100);
				}
