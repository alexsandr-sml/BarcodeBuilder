namespace nht.barcodes.Encoders.Code128
{
    using System;
    ...
    {
        public enum CODE128_TYPES:int { DYNAMIC, A, B, C };
        ...
        private CODE128_TYPES type = CODE128_TYPES.DYNAMIC;
        ...
        public Code128(string input, CODE128_TYPES type)
        {
            this.type = type;
            Raw_Data = input;
        }
        ...
        private void init_Code128()
        {
            ...
            C128_Code.Rows.Add(new object[] { "96", Convert.ToChar(202).ToString()/*FNC3*/, Convert.ToChar(202).ToString()/*FNC3*/, "96", "10111100010" });
            C128_Code.Rows.Add(new object[] { "97", Convert.ToChar(201).ToString()/*FNC2*/, Convert.ToChar(201).ToString()/*FNC2*/, "97", "11110101000" });
            C128_Code.Rows.Add(new object[] { "98", "SHIFT", "SHIFT", "98", "11110100010" });
            C128_Code.Rows.Add(new object[] { "99", "CODE_C", "CODE_C", "99", "10111011110" });
            C128_Code.Rows.Add(new object[] { "100", "CODE_B", Convert.ToChar(203).ToString()/*FNC4*/, "CODE_B", "10111101110" });
            C128_Code.Rows.Add(new object[] { "101", Convert.ToChar(203).ToString()/*FNC4*/, "CODE_A", "CODE_A", "11101011110" });
            C128_Code.Rows.Add(new object[] { "102", Convert.ToChar(200).ToString()/*FNC1*/, Convert.ToChar(200).ToString()/*FNC1*/, Convert.ToChar(200).ToString()/*FNC1*/, "11110101110" });
        ...
        private void BreakUpDataForEncoding()
        {
            ...
            if (this.type == CODE128_TYPES.A || this.type == CODE128_TYPES.B)
            {
                foreach (char c in Raw_Data)
                    _FormattedData.Add(c.ToString());
                return;
            }//if
            if (this.type == CODE128_TYPES.C)
            {
        ...
        private void InsertStartandCodeCharacters()
        {
            ...
            if (this.type != CODE128_TYPES.DYNAMIC)
            {
                switch (this.type)
                {
                    case CODE128_TYPES.A: _FormattedData.Insert(0, "START_A");
                        break;
                    case CODE128_TYPES.B: _FormattedData.Insert(0, "START_B");
                        break;
                    case CODE128_TYPES.C: _FormattedData.Insert(0, "START_C");
                        break;
        ...
        private string GetEncoding()
        {
            ...
            foreach (string s in _FormattedData)
            {
                ...
                switch (this.type)
                {
                    case CODE128_TYPES.A: E_Row = this.C128_Code.Select("A = '" + s1 + "'");
                        break;
                    case CODE128_TYPES.B: E_Row = this.C128_Code.Select("B = '" + s1 + "'");
                        break;
                    case CODE128_TYPES.C: E_Row = this.C128_Code.Select("C = '" + s1 + "'");
                        break;
                    case CODE128_TYPES.DYNAMIC: E_Row = this.C128_Code.Select("A = '" + s1 + "'");
